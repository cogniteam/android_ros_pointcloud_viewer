...
public class CogniPointCloud2DLayer extends SubscriberLayer<PointCloud2> implements TfLayer {
    ...
    public CogniPointCloud2DLayer(String topicName) {
        this(GraphName.of(topicName));
    }

    public CogniPointCloud2DLayer(GraphName topicName) {
    ...
    public void draw(VisualizationView view, GL10 gl) {
        if (vertexFrontBuffer != null) {
            synchronized (mutex) {
                Vertices.drawPoints(gl, vertexFrontBuffer, FREE_SPACE_COLOR, POINT_SIZE);
            }
        }
    }
...
//    Preconditions.checkArgument(pointCloud.getData().order().equals(ByteOrder.LITTLE_ENDIAN));
        final int size = (pointCloud.getRowStep() / pointCloud.getPointStep()) * 3 /* x, y, z*/;
...
//    vertexBackBuffer.put(0.f);
        ...
        while (buffer.readable()) {
            ...
            // Discard intensity.
            buffer.readFloat();
            //TODO: regard intensity
//            vertexBackBuffer.put(buffer.readFloat()); //intensity
        }
